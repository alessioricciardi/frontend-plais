...
const loadUsers = async (): Promise<void> => {
  try {
    users.value = await $fetch(`${API_URL}/api/admins`, {
      credentials: 'include',
    })
  } catch {
    useFetchError()
...
const updateUser = async (newUser: User): Promise<void> => {
  try {
    await $fetch(`${API_URL}/api/admins/${selectedUser.value}/reset-password`, {
      method: 'PUT',
      body: {
...
const deleteUser = async (): Promise<void> => {
  try {
    await $fetch(`${API_URL}/api/admins/${selectedUser.value}`, {
      method: 'DELETE',
      credentials: 'include',
    })
    loadUsers()
    ...
    <div class="flex h-full w-full flex-col">
      ...
      <div class="w-full flex-1 rounded-tl-2xl bg-gray-100">
        ...
        <UTable
          ...
          :columns="userColumns"
          :data="users"
          class="user-table h-full rounded-tl-2xl border-1 border-gray-200 bg-white shadow-xl lg:m-28 lg:h-auto lg:rounded-2xl"
          :ui="{
            thead: 'h-16',
            th: 'text-md',
            td: 'text-gray-900',
